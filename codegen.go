...
// generateGetState now uses encodingJson.Unmarshal
...
// generateSetState now uses encodingJson.Marshal
	...
	if len(m.Params) > 0 {
		if len(m.Params) == 1 && !isBasicType(m.Params[0].Type) {
			...
			for i, p := range m.Params {
				...
				sb.WriteString("params.")
				sb.WriteString(capitalizeFirst(p.Name)) // Ensure parameter access is capitalized
			}
		}
	}
	sb.WriteString(")\n\n")
...
func generateParamParser(method *MethodInfo) string {
	...
	if len(method.Params) == 1 && !isBasicType(method.Params[0].Type) {
		...
		for _, p := range method.Params {
			// Ensure field name is capitalized so it is exported and visible to encoding/json
...
// capitalizeFirst ensures the first letter of a string is uppercase
